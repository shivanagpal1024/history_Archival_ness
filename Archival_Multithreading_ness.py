...
def get_sys_argument():
    ...
    if len(sys.argv) == 2:
        ...
        if runtype.upper()=='M':
            ...
            msg=f"No valid Argument passed to drive the Program {sys.argv}"
            log.error(msg)
            ness_obj.post(msg=msg, name="get_system_args", severity=Severity.ERR.name, reason="nonSecurity",
...
def create_output_folder():
    ...
    reg_qual = os.getenv('region'.strip())[-3:]
...
def check_for_configuration():
    ...
    if len(error_list) != 0:
        msg=f"something went wrong while reading configuration=%s, error_list"
        log.error(msg)
        ness_obj.post(msg=msg, name="check_for_configuration", severity=Severity.ERR.name, reason="Security",
...
def db2_connection():
    ...
    '''
    dbase = os.getenv('database'.strip())
    region = os.getenv('region'.strip())
    hstnm = os.getenv('hostname'.strip())
    port = os.getenv('port'.strip())
    protocol = os.getenv('protocol'.strip())
    usrid = os.getenv('uid'.strip())
    pswd = os.getenv('pwd'.strip())
    try:
        ...
        msg=f"ERROR:Unable to connect to the DB2 Connection host=%s, username=%s, password=%s, database=%s, message=%s,",
        "hstnm, usrid, pswd, dbase, str(e)"
        log.error(msg)
        ness_obj.post(msg=msg, name="db2_connection", severity=Severity.ERR.name, reason="Security",
                      operation=Operation.CREATE.name)
        sys.exit(-1)
...
def ftp_connection():
    ...
    try:
        ...
        msg=f"ERROR:Something went wrong while creating FTP connection host=%s, username=%s, password=%s message=%s",
        "hst,usrid,pswd, str(e)"
        log.error(msg)
        ness_obj.post(msg=msg, name="ftp_connection", severity=Severity.ERR.name, reason="Security",
                      operation=Operation.CREATE.name)
...
def h_partn_xref(region):
    ...
    try:
        ...
        msg=f"Error:Unable to Fetch the result from H_PARTN_XREF table: {err}"
        log.error(msg)
        ness_obj.post(msg=msg, name="h_partn_xref", severity=Severity.ERR.name, reason="Security",
                      operation=Operation.CREATE.name)
        conn.close()
        conn.rollback()
...
def read_file(filename):
    ...
    if sys.argv[1]=='M':
        ...
        msg=f"No valid Argument passed to drive the Program={sys.argv}"
        log.error(msg)
        ness_obj.post(msg=msg, name="read_file", severity=Severity.ERR.name, reason="nonSecurity",
                      operation=Operation.READ.name)
        sys.exit(-1)
    mfilnm = filename[:-4]
...
def load_into_mysql(csvfilnm):
    ...
    ms_hst=os.getenv('mysql_host'.strip())
    ms_usr = os.getenv('mysql_user'.strip())
    ms_pswrd = os.getenv('mysql_password'.strip())
    ms_database = os.getenv('mysql_database'.strip())
    try:
        ...
        msg=f"something went wrong while creating connection host={ms_hst}, username={ms_usr}, password={ms_pswrd}, database={ms_database}, message={str(e)}",
        log.error(msg)
        ness_obj.post(msg=msg, name="load_into_mysql", severity=Severity.ERR.name, reason="Security",
                      operation=Operation.CREATE.name)
        sys.exit(-1)
